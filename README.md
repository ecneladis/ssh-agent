...
```console
docker run -d --name=ssh-agent ecneladis/ssh-agent:latest
```
...
```console
docker run --rm --volumes-from=ssh-agent -v ~/.ssh:/ssh -it ecneladis/ssh-agent:latest ssh-add /ssh/<host_key_file_name>
```
...
```console
docker run --rm --volumes-from=ssh-agent -v ~/.ssh:/ssh -it ecneladis/ssh-agent:latest cp /ssh/known_hosts /root/.ssh/known_hosts
```
